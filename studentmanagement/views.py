    ...
    def post(self, request):
        try:
            Student.objects.create(
                name=request.data["name"], roll_number=str(request.data["roll_number"])
            )
...
class StudentAttendentsDetails(APIView):
    def get(self, request, pk):
        try:
            ...
            student_attentence_serilizer = AttentenceSerializer(
                student_attentence, many=True
            )
            return Response(
                {"message ": student_attentence_serilizer.data},
            )
        except Exception as Error:
            return Response({"message": str(Error)})
    ...
    def post(self, request, pk):
        try:
            ...
            StudentAttendance.objects.create(
                ...
                leave_type=request.data.get("leave_type", None),
    ...
    def get(self, request):
        try:
            ...
            for student in students:
                student_attendence_present = attendence_details.filter(
                    student_id=student.id, is_present=True
                ).count()
                student_attendence_notpresent_fullday = attendence_details.filter(
                    student_id=student.id, is_present=False, leave_type="full_day"
                ).count()

                student_attendence_notpresent_halfday = attendence_details.filter(
                    student_id=student.id, is_present=False, leave_type="half_day"
                ).count()
                total_attendence = (
                    student_attendence_present
                    + student_attendence_notpresent_fullday
                    + (student_attendence_notpresent_halfday // 2)
                )
                attendence_percentage = (
                    student_attendence_present / total_attendence
                ) * 100
